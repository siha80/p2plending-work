...
public class SCrypt {
    /**
     * Reference: http://www.tarsnap.com/scrypt/scrypt-slides.pdf
     */
	private static final int N = 16 * 1024;
	private static final int r = 8;
	private static final int p = 1;
	private static int dkLen = 32;
	...
	*/
	public static String scrypt(String authPw, String salt) throws GeneralSecurityException {
		byte[] XY = new byte[256 * r];
		byte[] V = new byte[128 * r * N];
		int i;

		String newSalt = CryptoManager.pbkdf2(authPw, salt, 1, p * 128 * r);
        byte[] B = CryptoManager.hexToByteArray(newSalt);
		for (i = 0; i < p; i++) {
			smix(B, i * 128 * r, r, N, V, XY);
		}
		return CryptoManager.pbkdf2(authPw, B, 1, dkLen);
	}

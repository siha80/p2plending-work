...
import com.skp.payment.p2plending.user.domain.Token
import com.skp.payment.p2plending.user.ui.UsersApi
...
object launcher extends App with UsersApi {
  ...
  try {
    ...
    Http().bindAndHandle(handler = logRequestResult("log")(usersRoutes), interface = httpConfig.getString("interface"), port = httpConfig.getInt("port.user"))
  } catch {
